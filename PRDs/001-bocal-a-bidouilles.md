...
- **Time enforcement**: The app enforces deadline time in real-time. If a kid taps their avatar after the deadline, the check-in modal opens in a "late" state instead of showing the normal "I'm Ready!" flow. The check-in is still recorded (for audit/history), but it does not count toward success. Parents can retroactively correct any entry. The clock component only appears on the dashboard for goals that have a deadline time configured.
- **Late check-in UX**: When a kid is late (timed goal) or missed (untimed goal), the modal shows an encouraging message instead of the "I'm Ready!" button. The tone is kind and forward-looking -- never punitive or implying the child didn't try hard enough. The child taps an acknowledgment button to confirm they saw the message; this records the check-in as a miss. Messages are randomly selected from a pool of 4-5 variants per category to keep the experience fresh. Message categories are a 2x2 matrix:
  - **Timed + Individual**: "You were a little late today" style messages with personal encouragement
  - **Timed + Team**: "Your team was a little late" style messages emphasizing working together
  - **Untimed + Individual**: "You didn't get this one done today" style messages (used when parent marks as miss or end-of-day)
  - **Untimed + Team**: "Your team didn't get this one done" style messages
  All messages use "next time" (not "tomorrow" or "next week") to stay accurate regardless of which days are active. Button text is positive and action-oriented (e.g., "I can do it next time!", "We've got a plan!", "I'm not giving up!").
